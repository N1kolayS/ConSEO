...
$confString = ($model->value !== null) ? $model->value : '';
$script = <<<JS

    
     const confString = '$confString';
if (confString.length>1)
    {
        confObject = JSON.parse(confString);
        $("#name").val(confObject.name);
        $("#code").val(confObject.code);
        $("#url").val(confObject.url);
        $("#utm_campaign").val(confObject.utm_campaign);
        $("#utm_source").val(confObject.utm_source);
    }
    else {
    confObject = {};
    }
